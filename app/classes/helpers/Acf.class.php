...
class Acf {
	...
	static private function handle_field ($field_type, $field_output, &$return_field) {
		$carriage_return = "\r\n";
		$wrapper_tags 	 = [
			'p',
			'div',
...
*/
	...
	static private function get_field ($args, &$field) {
	...
	static private function get_image_alt (&$image) {
	...
	// get the image by id
	static private function get_image ($image_id, $size, &$image_data) {
		$image = new \stdClass();
	...
	static private function process_link_array (&$link) {
		if (isset($link['title'])) {
